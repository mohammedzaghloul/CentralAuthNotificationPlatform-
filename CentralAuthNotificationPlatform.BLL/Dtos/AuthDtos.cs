...
namespace CentralAuthNotificationPlatform.BLL.Dtos;
...
public sealed record VerifyRecoveryRequest(
    [Required, EmailAddress, MaxLength(256)] string Email,
    [Required, MinLength(6), MaxLength(32)] string RecoveryCode);

public sealed record RecoveryResetTokenResponse(
    string ResetToken);

public sealed record RecoveryResetPasswordRequest(
    [Required, MinLength(16), MaxLength(128)] string Token,
    [Required, MinLength(8), MaxLength(128)] string NewPassword);

public sealed record ResetRecoveryCodeRequest(
    [Required, MinLength(16), MaxLength(128)] string Token);

public sealed record ResetRecoveryCodeResponse(
    string NewCode);
